...
const controller = {
  ...
  products: (req, res) => {
    ...
    switch (product) {
      case 'camisetas':
        const tshirts = productsDB.filter(product => product.category == 'camisetas');
        products = tshirts;
        category = 'camisetas';
        break
      case 'hoddies':
        const hoddies = productsDB.filter(product => product.category == 'hoddies');
        products = hoddies;
        category = 'hoddies';
        break
      case 'mugs':
        ...
        break
      case 'cojines':
        const cojines = productsDB.filter(product => product.category == 'cojines');
        products = cojines;
        category = 'cojines';
        break;
      case 'stickers':
        const stickers = productsDB.filter(product => product.category == 'stickers');
        products = stickers;
        category = 'stickers';
        break
      case 'gorras':
        const gorras = productsDB.filter(product => product.category == 'gorras');
        products = gorras;
        category = 'gorras';
        break
      case 'botellas':
        const botellas = productsDB.filter(product => product.category == 'botellas');
        products = botellas;
        category = 'botellas';
        break
    }
